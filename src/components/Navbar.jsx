      ...
      <div className="max-w-7xl mx-auto px-3 sm:px-6 lg:px-8 h-16 flex items-center justify-between gap-2 sm:gap-4">
        ...
        <div className="flex items-center gap-2 sm:gap-3">
          ...
          <button
            ...
            aria-label="भाषा बदलें (Switch Language)"
            className="px-2.5 py-1.5 rounded-xl border border-rose-500/30 bg-rose-50 dark:bg-rose-950/40 text-rose-600 dark:text-rose-400 font-bold text-xs flex items-center gap-1.5 hover:scale-105 active:scale-95 transition shadow-sm"
          >
            <Globe className="w-3.5 h-3.5" />
            <span>{language === 'hi' ? '🇮🇳 हिंदी' : '🇬🇧 Eng'}</span>
          </button>

          {/* Interactive Theme Switch Button (Black / White) */}
          <button
            onClick={toggleTheme}
            aria-label="थीम बदलें (Switch Theme)"
            className="px-2.5 py-1.5 rounded-xl border border-slate-300 dark:border-slate-700 bg-slate-100 dark:bg-slate-800 text-slate-800 dark:text-slate-100 font-bold text-xs flex items-center gap-1.5 hover:scale-105 active:scale-95 transition shadow-sm"
          >
            {theme === 'dark' ? <Sun className="w-3.5 h-3.5 text-amber-400" /> : <Moon className="w-3.5 h-3.5 text-slate-700" />}
            <span className="hidden sm:inline">{theme === 'dark' ? '🌙 ब्लैक थीम' : '☀️ वाइट थीम'}</span>
          </button>
